...
class Controller {
  ...
  constructor(req, res, next) {
    ...
    if (this.redisClient) {
      this.cacheStore = new ControllerCache(
        this.redisClient,
        this.req._switchboard_controller,
        this.req.action
      ).cache;
    }
  ...
  permit (...keys) {
    ...
    for (i; i < len; i += 1) {
      let val = lodash.get(this.body, keys[i]);
      let isNotObject = !val || val.constructor !== Object
      if (isNotObject) {
        lodash.set(ref, keys[i], val);
      }
    }
  ...
  cache (options, fn) {
    ...
    let cache, hitFn, missFn;

    if (!this.redisClient) {
      throw new Error(`A redis client is required for ${this.req._switchboard_controller}`);
    }
    ...
    if (fn) {
      return cache.cachedVersion(this.req).then(result => {
        if (result) {
          this.res.set(this.cacheHitHeader, "true");
          return this.res.type(options.type || "json").send(result);
        } else {
          return fn.call(this, content => {
            return this.cacheStore.set(this.req.url, content);
          });
        }
      });
    } else {
          ...
          return new Promise((resolve, reject) => {
            cache.cachedVersion(this.req).then(result => {
              ...
              if (result && hitFn) {
                ...
                missFn(data => {
                  this.cacheStore.set(this.req.url, data);
                  resolve(data);
                });
              } else {
                this.res.set(this.cacheHitHeader, "false");
                reject({
                  err: `
  ...
  render(...args) {
    if (!this.viewPath) {
      throw new Error(`The view path is not set for ${this.req._switchboard_controller}`);
    }
...
Controller.prototype.cacheHitHeader = 'X-CACHE-HIT';
Controller.prototype.viewPath = false;
Controller.prototype.redisClient = false;
