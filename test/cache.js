...
describe('ControllerCache Controller Methods should work', function () {
  ...
  testController.prototype.redisClient = client;
  ...
  it('should cache on a request to a cacheable method (GET)', function () {
    let myController = new testController({controller: 'myController', action: 'Get', url: '/widgets'}, res);
  ...
  it('should cache on a request to a cacheable method (FindOne)', function () {
    let myController = new testController({controller: 'myController', action: 'FindOne', url: '/widgets'}, res);
  ...
  it('should remove a cached Item (PUT)', function () {
    let controllerForGet = new testController({controller: 'myController', action: 'Get', url: '/widgets'}, res);
    let controllerForPut = new testController({controller: 'myController', action: 'Put', url: '/widgets/12'}, res);

    return controllerForGet.Get()
      .then(() => controllerForGet.cacheStore.get('/widgets'))
      .then(value => assert.deepEqual(value, [{name: 'doc1'}]))
      .then(() => controllerForPut.Put())
      .then(() => controllerForGet.cacheStore.get('/widgets/12'))
  ...
  it('should remove a cached Item (DELETE)', function () {
    let controllerForGet      = new testController({controller: 'myController', action: 'Get', url: '/widgets'}, res);
    let controllerForFindOne  = new testController({controller: 'myController', action: 'FindOne', url: '/widgets/12'}, res);
    let controllerForDelete   = new testController({controller: 'myController', action: 'Delete', url: '/widgets/12'}, res);
...
describe('ControllerCache Environment Settings', function () {
  ...
  it('should not cache on a request to a cacheable method (GET) when cache is disabled', function () {
    let myController = new testController({controller: 'myController', action: 'Get', url: '/widgets'}, res);
    process.env.DISABLE_SWITCHBOARD_CACHE = true;
    return myController.Get()
      .then(x => (delete process.env.DISABLE_SWITCHBOARD_CACHE))
      .then(() => myController.cacheStore.get('/widgets'))
...
describe('lru', function () {
  ...
  it('get works', function () {
    let lruModule = lru(client, {max: 100});
    assert.doesNotThrow(() => lruModule.get('nope'));
  });

  it('get works with no key', function () {
    let lruModule = lru(client, {max: 100});
    assert.doesNotThrow(() => lruModule.get());
  });
