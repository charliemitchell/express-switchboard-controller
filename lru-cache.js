...
const defaultScore = () => new Date().getTime();
...
function buildCache(client, opts) {
  ...
  function namedKey (key = '') {
    return `${opts.namespace}-k-${key.toString()}`;
  }
  ...
  const get = key => {
    ...
    key = namedKey(key);
    const multi = client.multi().get(key);
    if (opts.increment) {
    ...
    return asPromise(multi.exec.bind(multi)).then(results => {
      if (results[0] === null && results[1]) {
        // value has been expired, remove from zset
        return asPromise(client.zrem.bind(client), ZSET_KEY, key).then(() => null);
      }
  ...
  const set = (key, value, maxAge) => {
    if (process.env.DISABLE_SWITCHBOARD_CACHE) return Promise.resolve();
    const score = -1 * opts.score(key);
    key = namedKey(key);
  ...
  const peek = key => {
    ...
    return asPromise(client.get.bind(client), key).then(result => {
      if (result === null) {
        // value may have been expired, remove from zset
        return asPromise(client.zrem.bind(client), ZSET_KEY, key).then(
          () => null
        );
      }
   ...
   */
  const reset = () =>
    asPromise(client.zrange.bind(client), ZSET_KEY, 0, -1).then(safeDelete);

  /*
   * Return true if the given key is in the cache
   */
  const has = key =>
    asPromise(client.get.bind(client), namedKey(key)).then(result => !!result);
   ...
   */
  const keys = () =>
    asPromise(client.zrange.bind(client), ZSET_KEY, 0, opts.max - 1).then(
      results => results.map(key => key.slice(`${opts.namespace}-k-`.length))
    );
   ...
   */
  const values = () =>
    asPromise(client.zrange.bind(client), ZSET_KEY, 0, opts.max - 1)
      .then(results => {
        const multi = client.multi();
